...
use axum::Json;
use game_fleet::ships::{load_ships_for_universe, ShipDef};
use serde::Serialize;
...
fn derive_movement_ship_stats(
    ship_type: &str,
    universe_ship_defs: &HashMap<String, ShipDef>,
) -> (f64, f64, f64) {
    universe_ship_defs
        .get(ship_type)
        .map(|ship| {
...
pub async fn helper_movement_handler(payload: Result<Json<Value>, JsonRejection>) -> Response {
...
pub async fn defense_rebuild_handler(payload: Result<Json<Value>, JsonRejection>) -> Response {
    let Json(body) = match payload {
        Ok(body) => body,
...
pub async fn attacker_distribution_handler(
...
pub async fn espionage_outcome_handler(payload: Result<Json<Value>, JsonRejection>) -> Response {
    let Json(body) = match payload {
        Ok(body) => body,
...
pub async fn mission_cargo_transfer_handler(
    payload: Result<Json<Value>, JsonRejection>,
) -> Response {
    let Json(body) = match payload {
        Ok(body) => body,
...
pub async fn harvest_collection_handler(payload: Result<Json<Value>, JsonRejection>) -> Response {
    let Json(body) = match payload {
        Ok(body) => body,
