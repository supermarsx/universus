...
async function loadMessages(folder) {
    ...
    try {
        ...
        loadingEl.style.display = 'none';
        emptyEl.style.display = 'block';
        emptyEl.textContent = 'Error loading messages';
    }
}
...
function getMessageTypeBadge(type) {
    const badges = {
        combat_report: '<span class="message-type-badge type-combat">COMBAT</span>',
        espionage_report: '<span class="message-type-badge type-espionage">SPY</span>',
        alliance_circular: '<span class="message-type-badge type-alliance">ALLIANCE</span>',
        system_notification: '<span class="message-type-badge type-system">SYSTEM</span>',
        player_message: '<span class="message-type-badge type-player">PLAYER</span>',
    };
    return badges[type] || '';
}
        ...
        <div class="message-viewer">
            ...
            <div class="message-actions">
                ${!message.isRead && currentFolder === 'inbox' ? 
                    '<button class="btn-mark-read" onclick="markAsRead(' + message.id + ')">Mark as Read</button>' : ''}
                ${message.fromUserId && currentFolder === 'inbox' ? 
                    '<button class="btn-reply" onclick="replyToMessage(' + message.id + ')">Reply</button>' : ''}
                <button class="btn-delete" onclick="deleteMessage(${message.id})">Delete</button>
                <button class="btn-secondary" onclick="closeMessageViewer()">Close</button>
            </div>
        </div>
    `;
...
function renderCombatReport(metadata) {
    ...
    return `
        <div style="margin-top: 20px;">
            <h4 style="color: #4fc3f7;">Combat Details</h4>
            <table class="combat-report-table">
                ...
                <tr>
                    ...
                    <td>${escapeHtml(metadata.attacker.username || 'Unknown')}</td>
                    <td>${metadata.attacker.shipsLost || 0}</td>
                    <td>-</td>
                </tr>
                <tr>
                    <td><strong>Defender</strong></td>
                    <td>${escapeHtml(metadata.defender.username || 'Unknown')}</td>
                    <td>${metadata.defender.shipsLost || 0}</td>
                    <td>${metadata.defender.defenseLost || 0}</td>
                </tr>
            </table>
            <p style="margin-top: 15px;">
                <strong>Result:</strong> ${metadata.winner === 'attacker' ? 'Attacker Victory' : 'Defender Victory'}
            </p>
            ${metadata.loot ? `
                <p><strong>Loot:</strong> 
                Metal: ${formatNumber(metadata.loot.metal || 0)}, 
                Crystal: ${formatNumber(metadata.loot.crystal || 0)}, 
                Deuterium: ${formatNumber(metadata.loot.deuterium || 0)}
                </p>
            ` : ''}
        </div>
    `;
...
function renderEspionageReport(metadata) {
    ...
    return `
        <div style="margin-top: 20px;">
            <h4 style="color: #4fc3f7;">Espionage Report</h4>
            <p><strong>Target:</strong> ${escapeHtml(metadata.target.username || 'Unknown')}</p>
            <p><strong>Planet:</strong> ${metadata.target.planet || 'Unknown'}</p>
            ${metadata.resources ? `
                <h5 style="color: #4fc3f7; margin-top: 15px;">Resources</h5>
                <p>
                    Metal: ${formatNumber(metadata.resources.metal || 0)}<br>
                    Crystal: ${formatNumber(metadata.resources.crystal || 0)}<br>
                    Deuterium: ${formatNumber(metadata.resources.deuterium || 0)}
                </p>
            ` : ''}
            ${metadata.fleet ? `
                <h5 style="color: #4fc3f7; margin-top: 15px;">Fleet</h5>
                <p>${JSON.stringify(metadata.fleet, null, 2)}</p>
            ` : ''}
            ${metadata.defense ? `
                <h5 style="color: #4fc3f7; margin-top: 15px;">Defense</h5>
...
async function handleCompose(e) {
    ...
    try {
        ...
        if (/^\d+$/.test(recipientInput)) {
            ...
            alert('Please enter a numeric user ID. Username lookup not yet implemented.');
            return;
        }
