...
| Scheduler/sharding consensus integration | `platform-scheduler`/`platform-sharding` crates are ready, and `app-scheduler-worker` now takes per-task leases via `platform-consensus`. | `app-sharding-worker` plus the rest of worker fleet still need consistent lease/runtime adoption through platform crates. |
