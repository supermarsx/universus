...
| --- | --- | --- |
| Worker adoption of platform runtime stack | `app-scheduler-worker` and `app-sharding-worker` now lease-guard cycles; `app-notifications-worker` now runs cleanup cycles via `platform-worker-runtime`. | The rest of the worker fleet still needs consistent `platform-worker-runtime` and platform routing/scheduler/sharding adoption. |
