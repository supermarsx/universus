...
| Scheduler/sharding consensus integration | `platform-scheduler`/`platform-sharding` crates are ready; `app-scheduler-worker` now boots through scheduler+routing+sharding APIs, while `app-sharding-worker` uses cycle lease guards. | `app-sharding-worker` still needs deeper use of `platform-sharding` decision/summary APIs and the remaining worker fleet needs consistent lease/runtime adoption. |
