...
| --- | --- | --- |
| Worker adoption of platform runtime stack | `app-scheduler-worker` and `app-sharding-worker` now lease-guard cycles; `app-notifications-worker`, `app-chat-worker`, `app-email-worker`, and `app-analytics-worker` run cycles/jobs via `platform-worker-runtime`. | The rest of the worker fleet still needs consistent `platform-worker-runtime` and platform routing/scheduler/sharding adoption. |
