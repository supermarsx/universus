...
| --- | --- | --- |
| Worker adoption of platform runtime stack | `app-scheduler-worker` now executes platform-scheduler handlers through `platform-worker-runtime`; `app-sharding-worker` is lease/catalog integrated; `app-notifications-worker`, `app-chat-worker`, `app-email-worker`, `app-analytics-worker`, and `app-bot-worker` run cycles/jobs via `platform-worker-runtime`. | The rest of the worker fleet still needs consistent `platform-worker-runtime` and platform routing/scheduler/sharding adoption. |
