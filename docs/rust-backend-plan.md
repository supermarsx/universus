...
| `platform-tenant-routing` *(planned)* | Maps tenant IDs to shard queues, enforces rate limits/backpressure and exposes hooks for tenant lifecycle events | `platform-tenancy`, `platform-observability`, `platform-cache` | new crate to keep fair multi-tenancy |
