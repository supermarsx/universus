...
- Baseline regression coverage now exists in `crates/platform-worker-runtime/tests/runtime_regression.rs` (leak counter reset, max in-flight backpressure, shutdown rejection, leased-task release semantics).  
- Continue expanding these tests as integration suites under `crates/platform-worker-runtime/tests` or as dedicated scripts referencing `platform-worker-runtime::tests`.  
