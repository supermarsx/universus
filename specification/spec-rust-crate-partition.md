...
| `platform-sharding` | Tracks shard metadata, node assignment, and lease-backed shard leaders | `platform-consensus`, `platform-tenancy`, `platform-db` | Drives `app-sharding-worker` and tenant scheduling decisions; crate now exists and exposes shard summaries + leader assignment APIs. |
| `platform-scheduler` | Job registration, cron metadata, tenant-aware retries, worker dispatch | `platform-sharding`, `platform-events`, `platform-consensus` | Now active; orchestrates job execution via `TenantRoutingDecision`s and shard summaries. |
| `platform-worker-runtime` | Thread/task pool utilities, graceful shutdown, leak detection, instrumentation | `tokio`, `platform-observability`, `platform-config` | Standardizes the runtime experience of chat/notifications/email/analytics workers and already runs the tenant instrumentation/testing harness. |
| `platform-adapter` | Unified adapter registry that injects tenant context, enforces consensus leases, exposes health | `platform-tenancy`, `adapter-db`, `platform-observability` | Wraps existing `adapter-db` + `adapter-provider-*` to centralize multi-database lifecycle; `PlatformAdapterRegistry` already gates adapters with JSON config + lease guards. |
...
3. **Scheduler/runtime plumbing** – Focus on integrating `platform-scheduler`, `platform-worker-runtime`, and `platform-adapter` (platform-tenant-routing and platform-sharding are already in place) so workers share consistent threading, tenant placement, job scheduling, and adapter lifecycle semantics.
...
6. **Tenant-routing validation** – Specify the `platform-tenant-routing` API (route summaries, quota permits, optional leases) and define a focused genus of tests (tenant isolation on shared executors, queue pacing/backpressure, lease expiration, route recomputation when metadata changes) so the crate can be implemented with measurable coverage.
