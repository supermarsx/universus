...
| Scheduler/sharder | `app-scheduler-worker`, `app-sharding-worker` | `rust-scheduler-worker`, `rust-sharding-worker` | Plan to rewire onto `platform-sharding`, `platform-scheduler`, and `platform-worker-runtime` for consistent threading/backpressure. |

## Platform Pillars
- **Multi-tenancy**: `platform-tenancy` stores `TenantContext`, enforces `TenantAccessLevel`, and automatically enriches logs/metrics with tenant IDs for every HTTP/queue/worker path. Additional crates (`platform-tenant-routing`, `platform-sharding`, `platform-worker-runtime`) share this context to keep tenants isolated.
...
## Next Steps
1. Build the planned runtime platform crates (`platform-tenant-routing`, `platform-sharding`, `platform-scheduler`, `platform-worker-runtime`, `platform-adapter`) so app workers consume shared utilities.
