...
| S-003 | `backend-core-napi` bridge source ownership | `crates/backend-core-napi` | `app-core-engine` + `platform-proto` | `game-combat`, `game-fleet` | [ ] Remove remaining runtime or test dependencies on N-API bridge.<br>[x] Remove crate from default workspace build graph (source retained; full deletion deferred until parity acceptance).<br>[ ] Archive compatibility notes in migration docs.<br>[ ] Delete crate source after final parity/SLO acceptance window. |
