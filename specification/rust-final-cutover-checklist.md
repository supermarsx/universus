...
| S-002 | Legacy Node backend runtime services (`backend`, `bot-service`, `admin-service`, `backend-core`) | `docker-compose.yml` | Rust service set (`app-api-gateway`, `app-admin-api`, `app-bot-api`, `app-core-engine`) | `app-realtime-gateway`, domain crates | [x] Legacy services moved behind explicit `legacy-node` compose profile.<br>[ ] Validate production rollout with `legacy-node` profile disabled.<br>[ ] Remove legacy services from compose after final rollback window. |
| S-003 | `backend-core-napi` bridge source ownership | `crates/backend-core-napi` | `app-core-engine` + `platform-proto` | `game-combat`, `game-fleet` | [ ] Remove remaining runtime or test dependencies on N-API bridge.<br>[ ] Delete crate from workspace and CI build graph after parity acceptance.<br>[ ] Archive compatibility notes in migration docs. |
