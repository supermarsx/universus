...
| S-004 | Notification orchestration and unread state | `backend/src/services/notificationService.ts` | `game-notifications`, `app-api-gateway` | `platform-db`, `app-realtime-gateway` | [x] Base notification domain crate created and integrated into Rust API gateway with protected CRUD/read-state routes.<br>[ ] Persist notification state in `platform-db` instead of in-memory store.<br>[ ] Emit realtime notification events through `app-realtime-gateway` and validate client contract snapshots.<br>[ ] Implement preference filtering and scheduled cleanup parity. |
